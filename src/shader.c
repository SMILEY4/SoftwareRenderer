...
void fshDefault(camera_t *camera, model_t *model, shader_t *shader, pixel_t *pixel, vec_t *iplPos, vec_t *iplUV, vec_t *iplNrm, vec_t *iplClr, vec_t *iplAttribs, uniformbuffer_t *uniformbuffer) {
    ...
    if(cosTheta < 0) {
        ...
        visibility = 1.0f - ((float)nHits / (float)nTests);
    ...
    float lambertian = fmaxf(0.0f, vecDot(&N, &L));
    float specular = 0.0f;
    ...
    vec_t finalColor = {
            ambient.x * pxDiffuse->r  +  pxDiffuse->r * lambertian * lightColor.x*lightColor.w  +  pxDiffuse->r * specular * lightColor.x*lightColor.w,
            ambient.y * pxDiffuse->g  +  pxDiffuse->g * lambertian * lightColor.y*lightColor.w  +  pxDiffuse->g * specular * lightColor.y*lightColor.w,
            ambient.z * pxDiffuse->b  +  pxDiffuse->b * lambertian * lightColor.z*lightColor.w  +  pxDiffuse->b * specular * lightColor.z*lightColor.w,
            0.0f
    };
